    ...
    async with sem:
        ...
        try:
            for speaker, group in groups:
                ...
                translated = await backend.translate_batch_async(
...
async def _run_gender_aware(
    ...
    *,
    ...
    """
    waveform, sr = await concurrency_mgr.run_in_thread(audio_obj.load_wav_mono, audio_path)
    ...
    try:
        for idx, chunk in enumerate(chunks):
            if precomputed_annotations is not None:
                ...
                genders = await concurrency_mgr.run_in_thread(
                ...
                annotation, genders = await concurrency_mgr.run_in_thread(
...
async def run_pipeline_async(
    ...
    *,
    ...
    """
    t0 = time.monotonic()
    segments = await concurrency_mgr.run_in_thread(transcribe.transcribe, audio_path, language)

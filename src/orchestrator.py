...
Web-agnostic. A future CLI calls ``Orchestrator.run_pipeline`` directly. Blocking ML work is
dispatched via ``concurrency_mgr.run_in_thread`` and gated by the caller's semaphore.
"""
...
class Orchestrator:
    ...
    async def run_pipeline(
        ...
        *,
        ...
        if settings.is_gender_aware():
            target_segments, annotations = await self._run_gender_aware(
                audio_path, segments, target,
                source_language=source_language,
            )
        else:
            target_segments = await self._run_plain_translate(
                segments, target,
                source_language=source_language,
            )
        ...
        if diff != 0:
            log.error(
                ...
                "_translate_chunk in src/orchestrator.py and the alignment fallback in "
    ...
    async def run_pipeline_alt_classifier(
        ...
        ``artifacts`` is the value the primary ``run_pipeline`` call
        ...
        request entering it mid-alt would observe the flipped value and use
        the wrong classifier on its own pipeline pass. The mutation is
        try/finally-restored, but that only protects the *post*-call state,
        not concurrent in-flight work.
        """
        ...
        try:
            ...
            if settings.is_gender_aware():
                ...
                target_segments, _ = await self._run_gender_aware(
                    audio_path, fresh_segments, target,
                    source_language=source_language,
    ...
    def _group_consecutive(
        self, items: list[tuple[Segment, str | None]]
    ...
    def _diarize_and_gender(self, audio: Any, sr: int, start: float, end: float):
        ...
        """
        import numpy as np  # noqa: F401 — kept for parity with original
        ...
        async with sem:
            ...
            try:
                for speaker, group in groups:
                    ...
                    translated = await self._backend.translate_batch_async(
    ...
    async def _run_gender_aware(
        ...
        *,
        ...
        """
        waveform, sr = await self._concurrency.run_in_thread(self._audio.load_wav_mono, audio_path)
        ...
        try:
            for idx, chunk in enumerate(chunks):
                if precomputed_annotations is not None:
                    ...
                    annotation, genders = await self._concurrency.run_in_thread(
                        self._diarize_and_gender, waveform, sr, chunk.start, chunk.end
                    )
                    log.info(

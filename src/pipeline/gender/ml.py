...
A thin alternative to the pitch-based ``pipeline/gender/pitch.py``. We wrap a
...
instance and only on first use.
...
class _Wav2Vec2GenderPipeline:
    """Module-private: external callers use ``GenderMLClassifier.classify_audio()`` instead.
...
class GenderMLClassifier:
    """wav2vec2 audio-classification singleton."""
    ...
    def get_pipeline(self) -> Any:
        ...
        with self._lock:
            ...
            model_id = self._settings.GENDER_ML_MODEL
            log.info(
            ...
            self._pipeline = _Wav2Vec2GenderPipeline(model, fe, device)
    ...
    def classify_audio(self, audio: np.ndarray, sr: int) -> tuple[str, float]:

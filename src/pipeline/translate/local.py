...
A drop-in alternative to ``pipeline.translate.claude`` (the Claude path), selected at
startup via ``settings.TRANSLATION_BACKEND``. The model is loaded once as a
per-``LocalTranslator``-instance singleton (same pattern as
``pipeline/transcribe.py``) and runs on GPU when available, falling back to
CPU otherwise.

The public method is named ``translate_batch_async`` and mirrors the
...
class LocalTranslator:
    """Local HuggingFace seq2seq translation singleton."""
            ...
            if _is_nllb_tokenizer(tokenizer):
                ...
                if target_code is None:
                    log.warning(
                        ...
                        "NLLB_LANGUAGE_CODES in pipeline/translate/local.py.",
                        target_language,
                    )
                else:
    ...
    async def translate_batch_async(
        ...
        Signature-compatible with ``pipeline.translate.claude.translate_batch_async`` so the
        ...
        unused: the local model is not an instruction-follower API and has no
        separate addressee-hint or scene-context pathway.
        """
        if not texts:
            return []
        return await asyncio.to_thread(
            self._translate_sync, texts, gender, target_language, source_language,
        )
